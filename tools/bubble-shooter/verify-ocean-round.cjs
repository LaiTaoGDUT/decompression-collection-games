...
try {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        r.futureRows=[];r.stage = 'boss-entry'; r.regionProgress = r.difficulty.progressRequired; roundtrip(r);
    ...
    r.futureRows=[];r.stage = 'boss'; r.board.reset([b(0,12), b(3,4,'red','tentacle'), b(3,5,'red'), b(4,4)]);
    ...
    assert(r.continueRegion('cloud')); assert.equal(r.snapshot().version,7);
