...
try{
 ...
 assert.equal(COLUMNS,14);assert.equal(BOARD_WIDTH,720);assert.equal(DIAMETER,720/14);assert.equal(TOP,410);assert(DANGER>-330);
 ...
 for(const version of [3,4]){
  ...
  assert(r.restore(old));assert.equal(r.snapshot().version,5);assert.equal(r.board.bubbles.length,145);assert(!r.board.danger);
 ...
 console.log('15-to-14 column migration preserves every bubble, color, support, inventory, progress, revive state; v5 roundtrip and malformed-save rejection passed.');
