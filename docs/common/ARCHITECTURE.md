...
每个主题定义以下运行时数据：物品 ID 有序目录、目标物品 ID、共享 GLB 中的稳定节点名、物品标签/颜色/尺寸倍率和标题装饰物品。收纳规则、三件合并、槽位容量、工具、计时、得分和存档字段保持不变。3D Mesh、共享材质/贴图及复合基础碰撞体配置必须位于 `game-catch-3d` Bundle 内，并与主题物品目录一一对应；动态刚体禁止使用三角形 `MeshCollider`。旧 4×5 图集和命中多边形仅作为原版 2D 运行时与 3D 造型参考，不进入 `game-catch-3d` 的运行时预加载或点选链路。
...
| `game-catch-3d` | 《桌面大清理》当前 3D 迁移版本的场景、规则、共享 GLB、3D 物理和复制后的主题 UI | 独立游戏分包 |
...
《桌面大清理》保留两个可独立加载的版本：`game-catch` 是原始 2D 兼容版本，`game-catch-3d` 是后续所有 3D 改动的唯一归属。3D 版本的纯 TypeScript 逻辑模型只保存物品 ID、类型、桌板/槽位归属、槽位顺序、分数、工具和局面状态；空间位置、旋转、速度、睡眠、碰撞和可见遮挡由 `game-catch-3d` 内的 `DesktopCleanupPhysicsWorld` 统一拥有。逻辑模型不得继续保存伪高度、平面速度、角速度或显示层，也不得由视图反推玩法结算。

桌面清理加载页沿用大厅清单中的独立封面，并在正式主题纹理全部加载完成前隐藏游戏根节点；初始化若发现主题资源不完整则进入统一加载错误态，不渲染旧序列化节点或程序化 fallback，避免进入时出现旧 UI 闪现。

场景拆分为独立世界层和 UI 层。正交 3/4 `WorldCamera` 渲染胡桃木背景、水平奶油色桌板、3D 物品和带正式纹理的 3D 槽体底板；`Canvas` 绑定的 UI Camera 在其后渲染七格槽位锚点、HUD、道具卡和弹窗。桌板继续保持原 1:1 方形外观、配色和屏幕占位，四周只设置略收于可视边缘的隐藏静态碰撞墙，不出现锅沿、盘沿或可见围栏。相机、Canvas 和 Layer 均由场景引用注入，业务代码禁止硬编码 Layer ID；响应式布局通过屏幕射线与桌面平面求交，把 UI 设计坐标投影到 3D 世界。

每局仍创建 54 个完整三件组，共 162 件，同组三件分散到不同出生波次。物品从奶油色桌板下方的隐藏出生口连续冒出，短促升到桌面并同步放大到完整尺寸，当前批次进入睡眠或达到安全超时后再投下一批；全部稳定前锁定拾取并保持既有暂停计时语义。穿透桌板、越界或速度异常的物品回收到桌板内安全高度重新投放。重排工具暂停倒计时并把剩余物品重新放入同一出生队列。

所有运行时物品使用共享 GLB、共享软陶材质/贴图和每类 1–3 个 Box、Sphere、Capsule 复合代理；动态物品禁止使用三角形 MeshCollider。低、中、高设备同时唤醒上限分别为 32、64、96：每次颠锅只唤醒最高、最外露且接近冲量方向的物件，深层睡眠刚体继续提供静态接触，稳定后重新休眠。暂停、规则页、广告、后台和退出冻结刚体并保存速度，恢复时只恢复原先活跃的刚体；`dispose()` 必须释放物理回调、动态材质实例、Mesh/Prefab 引用及 GPU 资源。

点选使用 `WorldCamera.screenPointToRay()` 和物理世界最近命中结果。只有真实碰撞体可被拾取：上层实体遮挡处命中上层，通过模型真实间隙可以命中下层。按下、移动持续更新候选，抬起时重新射线确认；候选高亮使用临时材质实例，取消、重建或销毁时必须恢复并释放。旧 Sprite Alpha、多边形命中和显示层排序不参与当前运行时。

颠锅只通过微信平台的 `Platform` 加速度计能力把设备运动转换为统一桌面冲量；游戏侧先滤除慢速姿态变化，在短窗口内累计有效运动量，达到阈值后只触发一个短促冲量并进入冷却，轻微移动不应推动物品。浏览器不模拟设备颠锅，手指移动仍只用于更新当前拾取候选。小游戏不得直接调用 `wx.*`；传感器监听必须在 `dispose()` 时取消，暂停、规则页、广告和退出期间不推进物理状态。

加速度计滤波、触发阈值、280ms 累计窗口和 240ms 冷却保持既有语义；每次有效动作只施加一次横向加向上的离散冲量，桌板自身不做可见倾斜或位移。浏览器不模拟设备颠锅，触摸移动只更新射线候选。小游戏仍只能通过 `Platform` 使用加速度计，不得直接调用 `wx.*`。
...
拾取、槽位压缩和三消继续使用既有表现事务与 operation generation 防重：关闭选中刚体碰撞后，同一个 3D 节点在 200ms 内飞入与七格中心投影一致的 3D 锚点；槽位压缩仍为 140ms；第三件完整入槽并稳定至少一帧后，三件按既有 240ms 汇聚，再播放原烟雾并提交逻辑移除。清除工具继续使用 55ms 错峰和 280ms 抬升、缩小、淡出；磁吸、继续和重排沿用原时序。动画看门狗必须与正常回调竞争同一提交 token，禁止重复释放或结算。

`gameplay.json` 升级为 schema v2，增加分批出生、睡眠、摩擦、反弹、回收阈值、颠锅冲量和分档唤醒参数；解析器继续接受 schema v1，并把旧配置映射到 v2 默认值。Manifest 保留 `catch` 的 `renderMode: 2d`，新增 `catch-3d` 并设置 `renderMode: 3d`、最低档 `low`。两套 Bundle 的视觉资源、场景脚本和存档命名空间相互隔离：原版使用 `catch`，3D 版使用 `catch-3d`；存档 `dataVersion` 和用户数据结构不变，无需迁移。需要回滚时只隐藏或移除 `catch-3d` Manifest 项，不改写或清空原版 `catch` 存档。

该游戏复用现有 `MiniGame`、`GameManifest`、`StorageService`、`AdService` 和 `FeedbackService` 契约，不新增全局服务，也不改变根存档 schema。原版数据写入 `catch` 命名空间，3D 版数据写入 `catch-3d` 命名空间，均使用 `dataVersion: 1`；首版只保存次数、最高分、最快通关、胜场、最后通关日期和规则已读版本，不保存中途局面。回滚时可从 Manifest 隐藏或移除 `catch-3d`，已有命名空间留存但不得清空其他游戏数据。

奖励广告按语义广告位路由，不得借用其他游戏的 placement。`AdService` 仍是唯一公共入口并继续负责广告开关、广告配置判断、串行化、状态机暂停/恢复和统计；应用层以 `RewardedAdRouter` 将三个 placement 分发到各自的微信 Provider。《桌面大清理》的工具加成和失败恢复共享 `desktop-cleanup-rewarded`，但在游戏模型内分别执行“一局一次成功工具广告奖励”和“一局一次成功失败恢复广告奖励”的频控：收纳槽失败清出最近 3 件，超时失败增加 60 秒；广告播放失败或中断不消耗对应机会，完整播放并发奖后才关闭该类入口；广告开关关闭时隐藏对应入口，开关开启但无广告配置时点击入口直接发放同一奖励，不改变频控语义。该调整只改变广告配置缺失时的处理时机，不改变 placement、奖励内容、存档 schema 或根存档；兼容影响仅限当前游戏会话，回滚时恢复失败即锁定的旧逻辑即可。
