...
玩法状态在游戏内部显式包含 `Failing` 和 `Resurrecting`，失败快照只保存在内存；当前不实现跨会话 `activeRound` 物理局面恢复。存档使用现有 `GameSaveData`：公共 `playCount/highScore/lastPlayedAt` 分别承载总局数、最高分和最后游玩时间，`custom.gameDataVersion = 2` 承载高度、击杀、射击统计、传感器设置、独立的 `tutorialCompleted` 引导完成标记和起步助推兼容库存，不保存复活库存或跨局复活次数。Playing 期间每 3 秒按“载入时固定基线＋本局绝对累计”重建并提交一次检查点，公共存储仍按自身 3 秒节流合并底层写入；暂停、微信 hide、退出、重开和销毁时立即 `flush()`。检查点会保住本局已经达到的 `highScore/bestHeightMeters/bestKillCount/totalShots/totalKills`，重复保存不会重复累加；它不包含角色瞬时位置、平台动态状态和复活次数，因此重新进入仍开始新局。`tutorialCompleted` 只在完整看完首次引导后写为 true，不以 `playCount` 推断。旧规划别名 `doodleJump` 只允许迁移到 `doodle-jump`，未知 `custom` 字段保留，历史规划中若已出现 `resurrectCount` 只能作为不再读取的未知兼容字段原样保留；迁移失败不得重置根存档。复活点只向上寻找：先检查失败相机当前可见玩法区内的安全平台，没有时保持物理与危险更新暂停，让复活流程驱动相机和生成器向上搜索；越过已预生成范围仍没有候选时，生成无危险附着的 Normal 安全平台作为确定性终止条件。该过程不回退高度、分数或生成游标，也不允许向下放置角色。
