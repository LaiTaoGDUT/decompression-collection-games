...
原预留的 `game-catch` 分包用于《桌面大清理》。游戏采用 2.5D 精灵堆叠而非真实 3D 物理：纯 TypeScript 模型负责每日种子、分层暴露、七格槽位、三件消除、道具、倒计时和结算；Cocos 场景只负责响应式布局、触摸、动画和主题呈现。这样可以保留“从物件堆中翻找”的核心触感，同时维持低档微信设备的内存、Draw Call 和释放边界。

每日布局由本地日期和版本盐派生，同日重开保持一致。生成器将 72 件物品组织为十二个紧凑遮挡层，每层 6 件并由两个完整三件组构成，目标徽章固定在最深层；运行前的批量自检保证布局数量、分组、紧凑中心跨度和深层目标约束成立。显示层级仍由纯模型的 `layer` 决定，但“能否点到”不采用全局最高层开关，也不允许矩形、椭圆或按物品类别手写近似热区：构建期从最终 SpriteFrame 的主体 Alpha 生成逐像素命中掩码，Cocos 输入层把触点逆变换到节点局部 UV 后，按实际显示顺序从最高节点向下查询统一阈值的实体像素。上层实体覆盖处优先命中上层，低层真实露出的实体像素仍可直接收取，透明像素、投影和完全被覆盖的物品不能截获输入。归位道具返回的物件显示在堆顶；重排道具把所有剩余物件重新压入紧凑多层堆，不修改根存档或公共协议。

堆放区的拖动采用连续视图状态而非抬手后离散重排：触摸开始时固化各物件起点，移动期间由当前最高非空层及其下方三层分别按 `1 / 0.58 / 0.30 / 0.14` 的权重跟随同一手指位移，第五层及更深层保持不动；偏移受紧凑堆边界约束，抬手后由视图层缓动回稳或固化合法偏移，不得瞬移。该手势不触发“上层物件拨开了一点”或同义提示，模型仍只接收最终合法位置结果，不持有 Cocos 节点。

槽位三消增加显式的待结算阶段。第三件物品先按普通入槽流程完成飞行动画并在槽内可见，随后三个对应槽位视图向共同中心汇聚；透明烟雾序列帧开始遮盖三件后，模型才原子提交三件移除、剩余槽位压缩、得分和终局判定。待结算期间锁定会改变槽位或结算状态的输入，动画取消、重开或退出通过现有 operation generation 失效保护收束，禁止由已销毁节点的回调重复结算。
...
奖励广告按语义广告位路由，不得借用其他游戏的 placement。`AdService` 仍是唯一公共入口并继续负责全局开关、串行化、状态机暂停/恢复和统计；应用层以 `RewardedAdRouter` 将三个 placement 分发到各自的微信 Provider。《桌面大清理》的工具加成和失败续玩共享 `desktop-cleanup-rewarded`，但在游戏模型内分别执行“一局一次工具加成”和“一局一次失败续玩”的频控；`catch` 开关关闭时，耗尽的工具变为不可交互，失败视图也不创建加时广告按钮。兼容影响仅限应用配置 v3，游戏存档和根存档不变；若回滚新游戏，可删除该路由项和清单注册，不影响其他游戏广告位或既有存档。
