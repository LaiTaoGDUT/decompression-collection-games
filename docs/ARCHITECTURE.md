...
《桌面大清理》的物品运动采用“模型内轻量 2D 运动解算 + 自定义伪 3D 遮挡”，不把 144 件物品全部交给 Cocos 动态刚体。模型为每件活动物品保存线速度、角速度、抬升量和近似露出状态；颠锅时露出物品获得更大的冲量，被压物品获得较小的桌面传导冲量；同一堆叠层内执行圆形代理碰撞、边界反弹、阻尼和旋转衰减。`layer + elevation` 只负责显示深度和翻开表现，不能替代可见性点选判断。渲染层每帧同步模型位置、角度、显示顺序和翻开脉冲，不持有玩法真相。
...
颠锅输入分为两条路径：微信平台通过 `Platform` 的加速度计能力把设备运动转换为统一桌面冲量，浏览器和不支持传感器的平台通过玩法区快速滑动模拟同一冲量。小游戏不得直接调用 `wx.*`；传感器监听必须在 `dispose()` 时取消，暂停、规则页、广告和退出期间不推进物理状态。
...
堆放区只保留贴合物件主体多边形的点选输入：透明区域、投影和完全被覆盖的物品不能截获点击。短距离手指移动只更新当前候选轮廓；移动超过颠锅阈值时转换为一次桌面冲量并取消本次拾取，避免滑动误触物品。
...
奖励广告按语义广告位路由，不得借用其他游戏的 placement。`AdService` 仍是唯一公共入口并继续负责全局开关、串行化、状态机暂停/恢复和统计；应用层以 `RewardedAdRouter` 将三个 placement 分发到各自的微信 Provider。《桌面大清理》的工具加成和失败恢复共享 `desktop-cleanup-rewarded`，但在游戏模型内分别执行“一局一次工具广告”和“一局一次失败恢复广告”的频控：收纳槽失败清出最近 3 件，超时失败增加 60 秒；`catch` 开关关闭时，耗尽的工具变为不可交互，失败视图也不创建广告恢复按钮。兼容影响仅限应用配置 v3，游戏存档和根存档不变；若回滚新游戏，可删除该路由项和清单注册，不影响其他游戏广告位或既有存档。
