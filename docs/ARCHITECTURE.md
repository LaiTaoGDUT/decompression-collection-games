...
每个主题定义以下运行时数据：物品 ID 有序目录、目标物品 ID、4×5 物品图集路径、物品标签/颜色/尺寸倍率和标题装饰物品。收纳规则、三件合并、槽位容量、工具、计时、得分、碰撞和存档字段保持不变。图集与命中多边形必须位于 `game-catch` Bundle 内，并与主题物品目录保持相同顺序；运行时只使用预生成的多边形，不读取 Alpha 掩码。
...
《桌面大清理》的物品运动采用“模型内轻量 2D 运动解算 + 自定义伪 3D 遮挡”，不把 162 件物品全部交给 Cocos 动态刚体。模型为每件活动物品保存线速度、角速度、抬升量和近似露出状态；颠锅冲量从当前最前活动层开始逐层衰减，被覆盖物品再叠加堆压衰减，极小的深层传导由静摩擦阈值吸收；线性运动使用高阻尼、低回弹以形成短促挪动。同一堆叠层内执行圆形代理碰撞、边界反弹、阻尼和旋转衰减。`layer + elevation` 只负责显示深度和轻微抬升缩放，不能替代可见性点选判断。初始化和动态边界都按物品旋转后的包围盒及桌板内边距计算，避免放大物品的角点溢出容器。渲染层每帧同步模型位置、角度和显示顺序，不持有玩法真相。
...
《涂鸦跃层》当前只完成规划，尚未进入正式 Bundle 实现。规划文件为 `docs/DOODLE_JUMP_GAMEPLAY_SPEC.md`，版本 v2.3；目标 Bundle 为 `game-doodle-jump`，Manifest `gameId` 固定为 `doodle-jump`，场景和入口分别为 `scenes/DoodleJump` 与 `DoodleJumpGame`。大厅只依赖 Manifest 和大厅展示副本，不加载游戏内纸片素材。
...
玩法状态在游戏内部显式包含 `Failing` 和 `Resurrecting`，失败快照只保存在内存；v2.3 不实现跨会话 `activeRound` 恢复。存档使用现有 `GameSaveData`：公共 `playCount/highScore/lastPlayedAt` 分别承载总局数、最高分和最后游玩时间，`custom.gameDataVersion = 2` 承载高度、击杀、射击统计、传感器设置和兼容道具库存。旧规划别名 `doodleJump` 只允许迁移到 `doodle-jump`，未知 `custom` 字段保留，迁移失败不得重置根存档；所有关键结算、退出和 Bundle 释放前调用同步 `StorageService.flush()`。

视觉方向冻结为“纸片跃层·平面涂鸦”：纯 2D 正交、正面纸片、铅笔轮廓和彩笔平涂；参考图的白边只表示平面轮廓，不实现厚度、偏移投影、透视、法线、景深或 3D 材质。纸飞机是攻击物的原创视觉替代，点击单发与长按连发共用 120ms 全局射击冷却；弹簧/蹦床的悬浮拾取与下一次落地触发、UFO/黑洞/护盾等精确数值也标记为本项目自定义，不宣称复刻原版隐藏实现。
