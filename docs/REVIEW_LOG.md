...
## 2026-08-13：广告前新增“转刀塔防”完成 Review

审查范围：第三款正式游戏范围变更、独立 Bundle/场景/Manifest、大厅封面、确定性模型、12 个内圈塔位、拖动调度、宠物合成、Bonus、宝箱成长、波次与生命、主题 UI/音频、生命周期、存档和定向构建验证。

结论：第 33～39 步完成，可在不接广告的前提下形成完整胜负闭环。

- 新增 `game-blade-defense` 微信分包和公开 `blade-defense` Manifest；大厅封面保留在 `lobby` Bundle，运行场景、规则、UI 与 9 个原创 MP3 均归属游戏 Bundle。
- 怪物沿圆环顺时针移动一圈并扣 1 点生命；共 12 波、10 点初始生命。模型按固定步长处理刀具旋转、同刀同目标命中冷却、伤害、死亡、掉落和结算。
- 12 个塔位全部等距位于怪物路线内侧；初始仅槽位 0 有 L1 宠物且伤害为 1。拖到空位移动、拖到异级交换、拖到同级合成，目标宠物身份和旋转相位保持稳定。
- 宠物固定 L1～L8，刀数、伤害、转速、攻击半径和冷却逐级变化。普通同级合成稳定 `+1`；30% 出现 Bonus，可稳妥或挑战 `+2 · 55%` / `+3 · 30%`，失败只保留目标塔位原等级宠物，非法跨级选项按等级裁剪。
- 首个 6 秒引导宝箱与前两次普通击杀掉箱保底消除前期零成长死锁，之后恢复 18% 随机掉落；满位奖励进入 FIFO 等待队列，腾位后自动领取。
- 前后台隐藏、主动暂停、Bonus 和结算统一冻结模型与音频；修复 App 在游戏加载期间切后台后可能直接恢复战斗的竞态，并让延迟加载的音乐遵守后台/游戏暂停锁。
- 开局只提交游玩次数和最近时间；最高分、最高宠物、最大波次和累计宝箱只在最终胜负结算提交，放弃本局不污染历史纪录。
- Cocos Creator 3.8.8 的 Web 手机端构建成功（1 分 6 秒），15MB 产物包含独立 `game-blade-defense/config.json`、`index.js` 和最新成长保底/存档字段；最终 TypeScript 定向编译、模型回归、Manifest/场景/资源检查均通过。浏览器能加载无新增控制台错误，但当前 App 启动/大厅构建基线只显示静态背景与品牌区，未把这次画面观察计作玩法闭环；微信真机与连续 30 次进退仍归入候选版本总体验收。

验证结果：`blade_defense_model=passed, levels=8, slots=12, geometry=passed, cooldown=passed, waves=passed, wave_scaling=passed, early_drop_pity=passed, chest_queue=passed, move_swap=passed, merge_target=passed, bonus_safe_success_failure=passed, gameover=passed`；`blade_defense_project=passed, manifest=public, slots=12, levels=8, audio=9, cover=920x690`。
